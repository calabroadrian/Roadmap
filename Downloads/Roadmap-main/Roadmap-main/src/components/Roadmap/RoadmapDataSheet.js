...
import AssignmentIcon from '@mui/icons-material/Assignment'; // Icono de tareas
...
const RoadmapDataSheet = ({ selectedItem, onEditItem, onAddItem, onSelectItem, onDeselectItem }) => {
  ...
  const [tabValue, setTabValue] = useState(0); // Tab index for filtering sprints
  ...
  const getTaskCountByStatus = (Estado) => {
    // Filtra los items por estado y sprint (si no se selecciona "Todos los Sprints", se filtra por el sprint seleccionado)
            ...
            <Paper elevation={0} sx={{ padding: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', justifyContent: 'space-between' }}>
                ...
                <Badge
                  ...
                  color="primary"
                  sx={{ marginLeft: 1 }} // Puedes ajustar esto si necesitas más espacio
                >
                  <AssignmentIcon />
                </Badge>
              </Box>
              ...
              {items
                .filter((item) =>
                  item.Estado === Estado &&
                  (tabValue === 0 || item.Sprint === sprints[tabValue - 1]) // Aplica el filtro de sprint solo si no está seleccionado "Todos los Sprints"
                )
                .map((item) => (
                  <Card
                    key={item.Id}
