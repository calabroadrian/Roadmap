...
const MyTimeline = ({ tasks }) => {
  ...
  // Items con fechas ajustadas por dependencias
  ...
  useEffect(() => {
    ...
    const newSvgs = dependencies.map(dep => {
      const fromDim = timelineRef.current.getItemDimensions(dep.fromItem);
      const toDim = timelineRef.current.getItemDimensions(dep.toItem);
      ...
      return (
        <svg key={`${dep.fromItem}-${dep.toItem}`} style={{ position: 'absolute', zIndex: 10, overflow: 'visible', pointerEvents: 'none', left: 0, top: 0 }}>
