  ...
  {selectedTask && (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button size="small" onClick={() => navigator.clipboard.writeText(selectedTask.id)}>Copiar</Button>
      </Box>
      <Typography variant="body2"><strong>Nombre:</strong> {selectedTask.name}</Typography>
      <Typography variant="body2"><strong>Inicio:</strong> {moment(selectedTask.start).format('DD/MM/YYYY')}</Typography>
      <Typography variant="body2"><strong>Fin:</strong> {moment(selectedTask.end).format('DD/MM/YYYY')}</Typography>
