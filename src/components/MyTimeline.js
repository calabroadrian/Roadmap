...
const MyTimeline = ({ tasks }) => {
  ...
  // Generar marcadores de dependencias y milestones
      ...
      if (dep) {
        markersElements.push(
          <CustomMarker
            ...
            style={{ left: 0 }}
            render={({ styles, dateAsString }) => (
              <svg style={styles} height="20">
    ...
    task.milestones.forEach((m, idx) => {
      markersElements.push(
        <CustomMarker
          ...
          style={{ left: 0 }}
          render={({ styles, dateAsString }) => (
            <Box style={styles} sx={{ position: 'absolute', top: 0, textAlign: 'center' }}>
