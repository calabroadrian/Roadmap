...
const MyTimeline = ({ tasks }) => {
  ...
  const groups = useMemo(
    ...
    const items = useMemo(
    () => safeTasks.map(task => {
      // Parse dependencias field into array
      let deps = [];
      if (Array.isArray(task.Dependencias)) {
        deps = task.Dependencias;
      } else if (typeof task.Dependencias === 'string') {
        const str = task.Dependencias.trim();
